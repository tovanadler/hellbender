...
public class BAQ {
     ...
     * @return
     */
    public static byte[] getBAQTag(MutableRead read) {
        String s = read.getAttributeAsString(BAQ_TAG);
        return s != null ? s.getBytes() : null;
    }

    public static String encodeBQTag(MutableRead read, byte[] baq) {
        ...
        for ( int i = 0; i < bqTag.length; i++) {
            ...
            if ( tag > Byte.MAX_VALUE )
                throw new UserException.MisencodedRead(read, "we encountered an extremely high quality score (" + (int)read.getBaseQualities()[i] + ") with BAQ correction factor of " + baq_i);
            bqTag[i] = (byte)tag;
        }
        return new String(bqTag);
    }

    public static void addBAQTag(MutableRead read, byte[] baq) {
    ...
    /**
      ...
      * @return
      */
    public static boolean hasBAQTag(MutableRead read) {
     ...
     *
     * @param read the MutableRead to operate on
     ...
     * @return
     */
    public static byte[] calcBAQFromTag(MutableRead read, boolean overwriteOriginalQuals, boolean useRawQualsIfNoBAQTag) {
     ...
     * @return
     */
    public static byte calcBAQFromTag(MutableRead read, int offset, boolean useRawQualsIfNoBAQTag) {
    ...
    public static class BAQCalculationResult {
        ...
        public BAQCalculationResult(MutableRead read, byte[] ref) {
     ...
     public BAQCalculationResult calcBAQFromHMM(MutableRead read, ReferenceDataSource refDS) {
...
//            logger.info("n = " + n + ": Total " + total.getElapsedTimeNano() + " local " + local.getElapsedTimeNano());
     ...
     * @return
     */
    private final Pair<Integer,Integer> calculateQueryRange(MutableRead read) {
    ...
    public BAQCalculationResult calcBAQFromHMM(MutableRead read, byte[] ref, int refOffset) {
     ...
     */
    public byte[] baqRead(MutableRead read, ReferenceDataSource refDS, CalculationMode calculationType, QualityMode qmode ) {
     ...
     * @return
     */
    public boolean excludeReadFromBAQ(MutableRead read) {
