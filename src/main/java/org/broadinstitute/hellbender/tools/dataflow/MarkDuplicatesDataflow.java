...
public final class MarkDuplicatesDataflow extends DataflowCommandLineProgram {
    ...
    private static final class MarkDuplicatesDataflowTransform extends PTransform<PCollection<Read>, PCollection<Read>> {
        ...
        */
        ...
        private final Map<String, ReadEndPhysicalLocation> readNamesToPhysicalLocations =
            new HashMap<>();
        ...
        private boolean isOpticalDuplicate(final PairedEnds pair, Iterable<PairedEnds> duplicatePairs) {
          ...
          ReadEndPhysicalLocation first = getPhysicalLocation(pair.first());
          ReadEndPhysicalLocation second = getPhysicalLocation(pair.second());

          for (final PairedEnds duplicatePair : duplicatePairs) {
            if (opticalDuplicateFinder.isOpticalDuplicate(getPhysicalLocation(duplicatePair.first), first) &&
                opticalDuplicateFinder.isOpticalDuplicate(getPhysicalLocation(duplicatePair.second), second)) {
              return true;
            }
          }
          return false;
                    ...
                    .of(new DoFn<KV<String, Iterable<PairedEnds>>, Read>() {
                        ...
                        public void processElement(final ProcessContext context) throws Exception {
                            ...
                            for (final PairedEnds pair : Iterables.skip(scored, 1)) {
                                if (opticalDuplicateFinder != null && isOpticalDuplicate(pair, Iterables.limit(scored, index))) {
