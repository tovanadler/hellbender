...
public class BaseRecalibrator extends ReadWalker {
    ...
    private boolean isLowQualityBase( final MutableRead read, final int offset ) {
    ...
    private static MutableRead consolidateCigar(MutableRead read) {
    ...
    private MutableRead resetOriginalBaseQualities(MutableRead read){
    ...
    private MutableRead setDefaultBaseQualities(MutableRead read) {
     ...
     */
    @Override
    public void apply( MutableRead originalRead, ReferenceContext ref, FeatureContext featureContext ) {
        ReadTransformer transform = makeReadTransform();
        final MutableRead read = transform.apply(originalRead);
        ...
        if( baqArray != null ) { // some reads just can't be BAQ'ed
            final ReadCovariates covariates = RecalUtils.computeCovariates(read, getHeaderForReads(), requestedCovariates);
    ...
    private boolean[] calculateSkipArray( final MutableRead read, final FeatureContext featureContext ) {
    ...
    protected boolean badSolidOffset( final MutableRead read, final int offset ) {
    ...
    protected boolean[] calculateKnownSites( final MutableRead read, final List<? extends Feature> features) {
    ...
    protected static int[] calculateIsSNP( final MutableRead read, final ReferenceContext ref, final MutableRead originalRead ) {
    ...
    protected static int[] calculateIsIndel( final MutableRead read, final EventType mode ) {
     ...
     */
    protected  static byte[] flatBAQArray(final MutableRead read) {
     ...
     */
    private byte[] calculateBAQArray( final MutableRead read ) {
