...
import org.broadinstitute.hellbender.tools.walkers.haplotypecaller.HaplotypeRoute;
import org.broadinstitute.hellbender.tools.walkers.haplotypecaller.Kmer;
import org.broadinstitute.hellbender.tools.walkers.haplotypecaller.graphs.MultiSampleEdge;
import org.broadinstitute.hellbender.tools.walkers.haplotypecaller.graphs.Route;
import org.broadinstitute.hellbender.tools.walkers.haplotypecaller.graphs.RouteFinder;
import org.broadinstitute.hellbender.utils.SequenceComplexity;
...
public class HaplotypeGraph extends ReadThreadingGraph {
     ...
     */
    private static final Logger logger = LogManager.getLogger(HaplotypeGraph.class);
    ...
    public boolean equals(HaplotypeGraph other) {
        ...
        if (!equalsReferencePaths(this, other)) return false;
        ...
        final List<Pair<MultiDeBruijnVertex,MultiDeBruijnVertex>> sourcePairs = equalsMatchVertexBySequenceMaps(thisSourcesBySequence, otherSourcesBySequence, unmatchedLeft, unmatchedRight);
        ...
        while (!pending.isEmpty()) {
            ...
            final MultiDeBruijnVertex leftVertex = pair.getLeft();
            final MultiDeBruijnVertex rightVertex = pair.getRight();
            final List<Pair<MultiDeBruijnVertex,MultiDeBruijnVertex>> childrenPairs = equalsMatchVertexBySequenceMaps(equalsBuildChildrenBySuffixMap(this, leftVertex),
            ...
            for (final Pair<MultiDeBruijnVertex,MultiDeBruijnVertex> childPair : childrenPairs) {
                final MultiDeBruijnVertex leftChild = childPair.getLeft();
                final MultiDeBruijnVertex rightChild = childPair.getRight();
                ...
                visited.add(childPair.getLeft());
                visited.add(childPair.getRight());
            }
        }
        return true;
    }
    ...
    private boolean equalsReferencePaths(final HaplotypeGraph g1, final HaplotypeGraph g2) {
    ...
    // Note: in practice only use by equals(HaplotypeGraph) for testing purposes.
    private static List<Pair<MultiDeBruijnVertex, MultiDeBruijnVertex>> equalsMatchVertexBySequenceMaps(
        ...
        for (final Map.Entry<String,MultiDeBruijnVertex> leftEntry : left.entrySet())
            if (right.containsKey(leftEntry.getKey()))
                result.add(new MutablePair<>(leftEntry.getValue(),right.get(leftEntry.getKey())));
            else
    ...
    private static Map<String, MultiDeBruijnVertex> equalsBuildSourceBySequenceMap(final HaplotypeGraph other) {
        ...
        for (final String s : collisions.keySet()) {
            ...
            for (final MultiDeBruijnVertex vertice : vertices)
                extendedSequences.add(new MutablePair<>(vertice, equalsCollisionResolverExtendedSequence(other, vertice)));
            ...
            for (final Pair<MultiDeBruijnVertex,String> p : extendedSequences)
                result.put(p.getRight() + '-' + (number++),p.getLeft());
        }
        return result;
    ...
    public HaplotypeGraph clone() {
        return (HaplotypeGraph) super.clone();
    }
}
