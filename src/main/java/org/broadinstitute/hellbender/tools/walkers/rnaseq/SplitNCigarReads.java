...
public class SplitNCigarReads extends CommandLineProgram {
    ...
    protected Object doWork() {
        ...
        final Iterable<MutableRead> readIter = new SAMRecordToReadIterator(in.iterator());
        final SAMFileReadWriter outputWriter = initialize(in);
    ...
    private SAMFileReadWriter initialize(final SamReader in) {
        final SAMFileHeader outputHeader = ReadUtils.cloneSAMFileHeader(in.getFileHeader());
        final SAMFileReadWriter outputWriter = new SAMFileReadWriter(new SAMFileWriterFactory().makeWriter(outputHeader, true, OUTPUT, REFERENCE_SEQUENCE));
     ...
     */
    public static MutableRead splitNCigarRead(final MutableRead read, OverhangFixingManager manager) {
     ...
     */
    private static MutableRead splitReadBasedOnCigar(final MutableRead read, final int cigarStartIndex, final int cigarEndIndex, final OverhangFixingManager forSplitPositions) {
