...
public class ArtificialReadUtilsUnitTest extends BaseTest {


    @Test
    public void basicReadIteratorTest() {
        Iterator<MutableRead> iter = ArtificialReadUtils.mappedReadIterator(1, 100, 100);
        int count = 0;
        while (iter.hasNext()) {
            MutableRead rec = iter.next();
            count++;
        }
    ...
    public void basicUnmappedIteratorTest() {
        Iterator<MutableRead> iter = ArtificialReadUtils.mappedAndUnmappedReadIterator(1, 100, 100, 1000);
        int count = 0;
