...
public final class AssemblyRegionUnitTest extends BaseTest {
    ...
    public void testConstructor(){
        ...
        final AssemblyRegion ar = new AssemblyRegion(loc, genomeLocParser, 2);
    ...
    public void testCreatingAssemblyRegions(final GenomeLoc loc, final List<ActivityProfileState> supportingStates, final boolean isActive, final int extension) {
        final AssemblyRegion region = new AssemblyRegion(loc, supportingStates, isActive, genomeLocParser, extension);
        ...
        assertGoodReferenceGetter(region.getAssemblyRegionReference(seq), region.getExtendedLoc(), 0);
        assertGoodReferenceGetter(region.getAssemblyRegionReference(seq, 0), region.getExtendedLoc(), 0);
        assertGoodReferenceGetter(region.getAssemblyRegionReference(seq, 10), region.getExtendedLoc(), 10);
        ...
        for ( final int start : Arrays.asList(1, 10, 100, contigLength - 10, contigLength - 1) ) {
            for ( final int readStartOffset : Arrays.asList(-100, -10, 0, 10, 100) ) {
                for ( final int readSize : Arrays.asList(10, 100, 1000) ) {
                    ...
                    if ( readLength > 0 ) {
                        final SAMRecord read = ArtificialSAMUtils.createArtificialRead(header, "read", 0, readStart, readLength);
    ...
    public void testAssemblyRegionReads(final GenomeLoc loc, final SAMRecord read) throws Exception {
        final GenomeLoc expectedSpan = loc.union(genomeLocParser.createGenomeLoc(read));

        final AssemblyRegion region = new AssemblyRegion(loc, null, true, genomeLocParser, 0);
        final AssemblyRegion region2 = new AssemblyRegion(loc, null, true, genomeLocParser, 0);
        Assert.assertEquals(region.getReads(), Collections.emptyList());
        Assert.assertEquals(region.size(), 0);
        ...
        region.removeAll(Collections.<SAMRecord>emptySet());
        Assert.assertEquals(region.getReads(), Collections.singletonList(read));
        Assert.assertEquals(region.size(), 1);
        Assert.assertEquals(region.getExtendedLoc(), loc);
        Assert.assertEquals(region.getReadSpanLoc(), expectedSpan);
        ...
        final SAMRecord read2 = (SAMRecord)read.clone();
        read2.setReadName(read.getReadName() + ".clone");

        for ( final SAMRecord readToKeep : Arrays.asList(read, read2)) {
            region.addAll(Arrays.asList(read, read2));
            final SAMRecord readToDiscard = readToKeep == read ? read2 : read;
    ...
    public void testBadReads(final SAMRecord read1, final SAMRecord read2) {
    ...
    public void testSplitAssemblyRegion(final GenomeLoc regionLoc, final List<GenomeLoc> intervalLocs, final List<GenomeLoc> expectedRegionLocs) {
        for ( final boolean addSubstates : Arrays.asList(true, false) ) {
            ...
            final AssemblyRegion region = new AssemblyRegion(regionLoc, states, true, genomeLocParser, 0);
    ...
    public void testTrimAssemblyRegion(final GenomeLoc regionLoc, final int extension, final GenomeLoc desiredSpan, final GenomeLoc expectedAssemblyRegion, final int expectedExtension) {
        final AssemblyRegion region = new AssemblyRegion(regionLoc, Collections.<ActivityProfileState>emptyList(), true, genomeLocParser, extension);
        final AssemblyRegion trimmed = region.trim(desiredSpan);
        Assert.assertEquals(trimmed.getLocation(), expectedAssemblyRegion, "Incorrect region");
