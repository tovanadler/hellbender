...
public final class GroupReadsWithRefBasesTest {
    ...
    public Object[][] bases(){
        ...
        List<KV<ReferenceBases, Iterable<Read>>> expected = Lists.newArrayList(
                ...
                KV.of(FakeReferenceSource.bases(makeInterval(rStartLength.get(2))), Lists.newArrayList(inputs.get(2)))
        );

        List<KV<Read, ReferenceBases>> kvs1 = Lists.newArrayList(
                KV.of(inputs.get(0), getBases(inputs.get(0).getStart(), inputs.get(0).getEnd())),
                KV.of(inputs.get(1), getBases(inputs.get(1).getStart(), inputs.get(1).getEnd())),
                KV.of(inputs.get(2), getBases(inputs.get(2).getStart(), inputs.get(2).getEnd()))
        );

        return new Object[][]{
                {inputs, kvs, expected, kvs1, longerInterval, shorterInterval},
        };
    }
    ...
    public void refBasesTest(List<Read> reads, List<KV<ReferenceShard, Iterable<Read>>> kvs, List<KV<ReferenceBases, Iterable<Read>>> expected,
                             List<KV<Read,ReferenceBases>> kvs1, SimpleInterval longInterval, SimpleInterval shortInterval) {
        ...
        // We have to use GroupReads and RefBasesFromAPI.GetBases because if we try to create a pCollection that
        // contains an ArrayList, we get a coder error.
        ...
        PCollection<KV<Read, ReferenceBases>> result = kvpCollection.apply(new GroupReadWithRefBases());
